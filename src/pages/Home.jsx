...
//   // In-memory storage for profiles (temporary, resets on page refresh)
//   const [profiles, setProfiles] = useState([]);

//   const handleSubmit = (e) => {
//     e.preventDefault();
//     try {
//       // Add new profile to the in-memory array
//       const newProfile = { ...formData, _id: Date.now().toString() }; // Unique ID based on timestamp
//       const updatedProfiles = [...profiles, newProfile];
//       setProfiles(updatedProfiles);

//       // Create Excel file
//       const worksheet = XLSX.utils.json_to_sheet(updatedProfiles);
...
//       alert('Profile submitted and Excel file downloaded!');
//       setFormData({
//         name: '',
//         dob: '',
//         email: '',
...
function Home() {
  ...
  const handleSubmit = (e) => {
    ...
    try {
      ...
      if (existingProfiles.some((p) => p.email === formData.email || p.phone === formData.phone)) {
        alert('Profile with this email or phone already exists!');
        return;
      }

      // Add new profile
      const newProfile = { ...formData, _id: Date.now().toString() };
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          <div className="w-full lg:w-1/2 bg-white rounded-xl shadow-md p-6 lg:p-8 animate-slide-up">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-accent text-white font-bold py-4 rounded-xl shadow-md transition-all hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-accent"
              >
                Submit
              </button>
            </form>
